...
elif np.array(trainstation).shape[0]==1:
	yearname = "2013-17"

# 修改位置处
file_path = r"D:\data\lstSimulate\model\\"
model_file_path  = file_path + str(yearname) +"_"+ str(trainstation[0]) + "_model.m"
# model_file_path  = file_path + "2013-17_SDQ_model.m"
# 输出结果excel保存位置
save_excel_path = r"D:\data\lstSimulate\Result\LSTM\4\\"

# RMSE\R2\BIAS 保存位置
save_RMSE_path = r"D:\data\lstSimulate\Result\LSTM\indicator\4\4_" +str(trainstation[0]) +'_'+str(teststation[0]) + r"\\RMSE.xlsx"
save_r2_path = r"D:\data\lstSimulate\Result\LSTM\indicator\4\4_" +str(trainstation[0]) +'_'+str(teststation[0])+ r"\\r2.xlsx"
save_MAE_path = r"D:\data\lstSimulate\Result\LSTM\indicator\4\4_" +str(trainstation[0]) +'_'+str(teststation[0])+ r"\\MAE.xlsx"
save_bias_path = r"D:\data\lstSimulate\Result\LSTM\indicator\4\4_" +str(trainstation[0]) +'_'+str(teststation[0])+ r"\\bias.xlsx"
bias_save = []
RMSE_save = []
r2_save = []
MAE_save = []
...
for i in testyear:
	for station in teststation:
		...
		print(trainyear, trainstation, '+', i, station, rmse, r2, bias)

		# print(trainyear,trainstation,'+',i,station, 'Root Mean Squared Error:',rmse,r2,bias)

		# 画图检验
		plt.plot(y_test , 'b', label='real', linewidth=1)
		plt.plot(y_pred , 'r', label='prediction', linewidth=1)
		# print(s,miny)
		plt.legend(loc='best')
		plt.ylim((250, 350))
		# plt.show()
...
print(trainyear, trainstation, '+all',rmse,r2,bias)
# # 画图检验
